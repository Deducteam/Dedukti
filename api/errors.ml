...
let of_typing_error red err : error_msg =
  ...
  | UnsatisfiableConstraints (r,(q,t1,t2)) ->
    108, Some (Rule.get_loc_rule r), Format.asprintf
  ...
  | BracketExprBoundVar (te,ctx) ->
    109, Some (get_loc te), Format.asprintf
...
let of_signature_error red err =
  ...
  | UnmarshalBadVersionNumber (lc,md) ->
    303, Some lc, Format.asprintf "Fail to open\ module '%a' (file generated by a different version?)." pp_mident md
  | UnmarshalSysError (lc,md,msg) ->
    304, Some lc, Format.asprintf "Fail to open module '%a' (%s)." pp_mident md msg
