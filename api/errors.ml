...
let fail_exit md code code lc fmt =
  let eid = red ("[ERROR:" ^ code ^ "] ") in
  begin match lc with
...
let fail_typing_error env code def_loc err =
  ...
  let fail lc = fail_exit md 3 code (Some lc) in
  let open Typing in
  match err with
  | KindIsNotTypable ->
    fail def_loc
...
let fail_signature_error env code def_loc err =
  let fail lc = fail_exit (Env.get_name env) 3 code (Some lc) in
