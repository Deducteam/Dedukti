...
struct
  ...
  let handle_entry e =
    ...
    match e with
    | Decl(lc,id,scope,locality,st,ty) ->
      Signature.add_external_declaration sg lc (Basic.mk_name md id) scope locality st ty
    | Def(lc,id,scope,locality,_,Some ty,te) ->
      let open Rule in
      Signature.add_external_declaration sg lc (Basic.mk_name md id) scope locality Signature.Definable ty;
