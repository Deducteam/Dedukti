...
let mk_prelude lc name =
  eprint lc "Module name is '%a'." Pp.pp_ident name;
  Env.init name;
  Confluence.initialize ()

let mk_declaration lc id pty : unit =
  eprint lc "Declaration of constant '%a'." Pp.pp_ident id;
  match Env.declare_constant lc id pty with
    | OK () -> ()
    | Err e -> Errors.fail_env_error e

let mk_definable lc id pty : unit =
  eprint lc "Declaration of definable '%a'." Pp.pp_ident id;
  match Env.declare_definable lc id pty with
    | OK () -> ()
    | Err e -> Errors.fail_env_error e

let mk_definition lc id pty_opt pte : unit =
  eprint lc "Definition of symbol '%a'." Pp.pp_ident id ;
  match Env.define lc id pte pty_opt with
    | OK () -> ()
    | Err e -> Errors.fail_env_error e

let mk_opaque lc id pty_opt pte =
  eprint lc "Opaque definition of symbol '%a'." Pp.pp_ident id ;
...
let mk_rules = function
  ...
  | ((_,pat,_)::_) as lst ->
    begin
      ...
      eprint l "Adding rewrite rules for '%a.%a'" Pp.pp_ident md Pp.pp_ident id;
      match Env.add_rules lst with
      | OK lst2 ->
        List.iter ( fun (ctx,pat,rhs) ->
            eprint (Rule.get_loc_pat pat) "%a" Pp.pp_typed_rule (ctx,pat,rhs)
          ) lst2 ;
      | Err e -> Errors.fail_env_error e
    end
...
let mk_ending () =
  ( if !export then
    if not (Env.export ()) then
      Errors.fail dloc "Fail to export module '%a'." Pp.pp_ident (Env.get_name ()) );
  Confluence.finalize ()
