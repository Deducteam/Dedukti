...
let in_deps : string -> bool = fun n -> List.mem_assoc n !current_deps

let add_dep : string -> string option -> unit =
 fun name file ->
  let cmp (s1, _) (s2, _) = String.compare s1 s2 in
  match file with
  | None -> ()
  | Some file -> current_deps := List.sort cmp ((name, file) :: !current_deps)
...
let find_dk : string -> string list -> string option =
 fun name path ->
  ...
  | [f] -> Some f
  | fs ->
      Printf.eprintf "Several files correspond to module %S...\n" name ;
...
let add_dep : string -> unit =
 fun name ->
  if name = !current_mod || in_deps name then ()
  else add_dep name (find_dk name (get_path ()))


(** Term / pattern / entry traversal commands. *)

let mk_name c = add_dep (string_of_mident (md c))
...
let mk_rule r = mk_pattern r.pat ; mk_term r.rhs
...
let handle_file : string -> dep_data =
 fun file ->
  try
    (* Initialisation. *)
...
let output_deps : out_channel -> dep_data list -> unit =
 fun oc data ->
  let objfile src = Filename.chop_extension src ^ ".dko" in
  let output_line : dep_data -> unit =
   fun (name, (file, deps)) ->
...
let output_sorted : out_channel -> dep_data list -> unit =
 fun oc data ->
