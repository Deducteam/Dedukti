...
let mk_entry md e =
  ...
  | Name (_, n) ->
      if not (mident_eq n md) then warn "Invalid #NAME directive ignored.\n%!"
  | Require (lc, md) ->
...
let mk_entry beautify md =
  if beautify then Pp.print_entry Format.std_formatter else mk_entry md
...
let _ =
  ...
  let options =
    Arg.align
      [ ("-d", Arg.Int Basic.set_debug_mode, "N sets the verbosity level to N")
      ; ( "-v"
      ...
      ; ("-nc", Arg.Clear Errors.color, " Disable colors in the output")
      ; ( "-stdin"
