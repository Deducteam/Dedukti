...
let mk_prelude (l,v : loc*string) : unit =
        Global.vprint ( Error.string_of_loc l ^ "[Name] " ^ v ^ ".\n") ;
        Global.set_name v ;
        Env.init v

let mk_require (l,v : loc*string) : unit = 
        Global.vprint ( Error.string_of_loc l ^ "[Import] " ^ v ^ " (This is obsolete !).\n") ;
        Env.import l v

let mk_declaration ((l,id),pty : (loc*string)*pterm) : unit = 
        let ty = Pterm.of_pterm [] pty in
        Global.vprint ( Error.string_of_loc l ^ "[Declaration] " ^ id ^ ".\n" ) ;
...
let mk_definition ((l,id),pty,pte : (loc*string)*pterm*pterm) : unit = 
        ...
        Global.vprint ( Error.string_of_loc l ^ "[Definition] " ^ id ^ ".\n") ;
        check_type (get_loc pty) [] ty ;
...
let mk_infered_def ((l,id),pte : (loc*string)*pterm) : unit =
        let te = Pterm.of_pterm [] pte in
        Global.vprint ( Error.string_of_loc l ^ "[Infered Definition] " ^ id ^ ".\n") ;
...
let mk_opaque ((l,id),pty,pte : (loc*string)*pterm*pterm) : unit = 
        let ty = Pterm.of_pterm [] pty in
        let te = Pterm.of_pterm [] pte in
        Global.vprint ( Error.string_of_loc l ^ "[Opaque] " ^ id ^ ".\n") ;
        check_type (get_loc pty) [] ty ;
        check_term (get_loc pte) te ty ;
        Env.add_decl l id ty 
...
let mk_typecheck (l,pty,pte : loc*pterm*pterm) :unit = 
        let ty = Pterm.of_pterm [] pty in
        let te = Pterm.of_pterm [] pte in
        Global.vprint ( Error.string_of_loc l ^ "[TypeCheck] _ \n") ;
        check_type (get_loc pty) [] ty ;
        check_term (get_loc pte) te ty
...
let mk_rules (lst:prule list) : unit = 
        ...
        in
        let (l,v) = aux lst in
        Global.vprint (Error.string_of_loc l ^ "[Rewrite] " ^ v ^ ".\n") ; 
