...
let is_type lc te = function
    | Type      -> ()
    | ty        -> raise (TypingError (lc,Error.err_conv te Type ty))
  ...
  match te with
    ...
    | App ( f::((_::_) as args) )       ->
        begin
          snd ( List.fold_left (
            fun (f,ty_f) u ->
              match Reduction.hnf ty_f , infer lc ctx u with
                | ( Pi (a,b) , a' ) ->  
...
let rec term_of_pattern = function 
  | Var n                       -> DB n
  | Dash n                      -> Meta n
  | Pattern ((_,m,v),[||])      -> GVar (m,v)
  | Pattern ((_,m,v),args)      -> App( GVar (m,v) :: (List.map term_of_pattern (Array.to_list args)) )
