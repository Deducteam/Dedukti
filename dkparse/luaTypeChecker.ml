...
let init name = 
  ...
  let ls = LuaL.newstate () in
    LuaL.openlibs ls ; 
    assert (LuaL.dostring ls "require(\"jit\")") ;
    assert (LuaL.dostring ls "require(\"dedukti\")") ;
(*    force_true (LuaL.dostring ls "require(\"profiler\")") ;*)
(*    force_true (LuaL.dostring ls ("profiler.start()") ) ;*)
    assert (LuaL.dostring ls (name^" = { }")) ;
...
let rec lua_code bf = function
  ...
  | GVar v              -> ( add_string bf "app0("  ; add_string bf !Global.name ; add_string bf "." ; add_string bf v ; add_string bf "_c)" )
  | EVar v              -> ( add_string bf "app0("  ; add_string bf v ; add_string bf "_c)" )
  | Var v               -> ( add_string bf "app0("  ; add_string bf v ; add_string bf "_c)" )
  | App (f,a)           -> ( add_string bf "app( "  ; lua_code bf f   ; add_string  bf  " , " ; lua_code bf a  ; add_string  bf  " )"    )
  | Lam (v,_,te)        -> ( add_string bf "{ co = clam ; f = function (" ; add_string bf v ; add_string bf "_c) return " ; lua_code bf te ; add_string  bf " end }" )
  | Pi (v0,ty,te)       ->
        begin
          add_string  bf "{ co = cpi ; ctype = " ; 
          lua_code bf ty ;
          add_string  bf " ; f = function (" ;
          (match v0 with 
             | Some v -> ( add_string bf v ; add_string bf "_c" ) 
             | None   -> add_string bf "dummy" );
...
let rec lua_term bf = function
  ...
  | GVar v              -> ( add_string bf !Global.name ; add_string bf "." ; add_string bf v ; add_string bf "_t" )
  | EVar v              -> ( add_string bf v ; add_string bf "_t" )
  | Var v               -> ( add_string bf v ; add_string bf "_t" )
  | App (f,a)           -> 
      begin 
  ...
  | Lam (v,None,te)     -> 
      begin
        add_string  bf "{ te = tlam ; ttype = nil ; ctype = nil ; f =  function (" ;
        add_string bf v ;
        add_string bf "_t, " ;
        add_string bf v ;
        add_string bf "_c) return " ; 
        lua_term bf te;
        add_string  bf " end }";
      end
  | Lam (v,Some ty,te)  -> 
  ...
  | Pi  (ov,ty,t)       -> 
        begin 
          ...
          add_string  bf " end ; f = function (" ;
          (match ov with 
             | None   -> add_string bf "dummy1,dummy2" 
             | Some v -> ( add_string bf v ; add_string bf "_t," ; add_string bf v ; add_string bf "_c" ) );
          add_string bf ") return " ;
          lua_term bf t ;
          add_string  bf " end }"
      end
...
let check_type ls te ty = 
  let bf = create 4096 in (*FIXME 80?*)
...
let rec lua_pat_term bf = function 
  | Id v                -> ( add_string bf v ; add_string bf "_t" )
  | Pat (c,dots,pats)   -> 
      let arity = Array.length dots + Array.length pats in
...
let check_env bf (id,te) = 
  ...
  add_string bf "local " ;
  add_string bf id ;
  add_string bf "_t = { te = tbox, ctype = function() return " ;
  lua_code   bf te ;
  add_string bf  " end }\n"
...
let print_path bf p = 
    ...
    iteri ( 
      fun i e ->
        if i=0 then ( add_string bf "y" ; add_string bf (string_of_int (e+1)) )  
        else ( add_string bf ".args[" ; add_string bf (string_of_int (e+1)) ; add_string bf "]" )
    ) (List.rev p) (*FIXME*)
  ...
  if Array.length vars = 0 then ()
    ...
    begin
      let first = ref true in
        ...
        Array.iter (
          function 
            | Id id   -> if !first then ( add_string bf id ; add_string bf "_c" ; first:=false ) else ( add_string bf ", " ; add_string bf id ; add_string bf "_c" )
            | _       -> assert false
        ) vars;
        first := true;
        add_string bf " = ";
            ...
            List.iter ( 
              fun (cst,arity,dots,lst) ->
                ...
                match specialize pm c arity dots lst with
                  | None        -> ( add_string bf "return " ; lua_code bf (snd pm.a.(match lst with z::_ -> z | _ -> assert false)) )
...
let add_rules ls id rules = 
  ...
  let bf = create 4096 in (*FIXME 80?*)
    ...
    (for i=2 to arity do ( add_string bf ", y" ; add_string bf (string_of_int i) )  done );
    add_string bf ")\n" ;
