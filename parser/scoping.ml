...
let get_vars_order (vars: pcontext) (ppat: prepattern) : untyped_context * bool =
  ...
  let is_a_var id1 =
    ...
    aux vars
  in
  let rec aux (bvar: ident list) (ctx: (loc * ident) list)
      : prepattern -> untyped_context = function
...
let scope_rule md ((l, pname, pctx, md_opt, id, pargs, pri): prule)
  ...
  if unused_vars then
    warn "Local variables in the rule %a are not used (%a)" pp_prule
      (l, pname, pctx, md_opt, id, pargs, pri) pp_loc l ;
  let idents = List.map snd ctx in
  let b, id =
    match pname with
    | None ->
        let id =
          Format.sprintf "%s!%d" (string_of_ident id) (fst (of_loc l))
        in
        (false, mk_ident id)
    | Some (_, id) -> (true, id)
  in
  let name =
    let md =
      match pname with Some (Some md, _) -> md | _ -> Env.get_name ()
    in
    Gamma (b, mk_name md id)
  in
  {name; ctx; pat= p_of_pp md idents top; rhs= t_of_pt md idents pri}
