...
and pp_prepattern_wp fmt = function
  | (PLambda (_, _, _) | PPattern (_, _, _, _ :: _)) as p ->
      fprintf fmt "(%a)" pp_prepattern p
...
let pp_pcontext fmt ctx =
  pp_list ".\n"
    (fun out (_, x) -> fprintf fmt "%a" pp_ident x)
    fmt (List.rev ctx)
...
let pp_prule fmt ((_, pname, pdecl, pid, id, prepatterns, prete): prule) : unit =
  ...
  | Some m ->
      ...
      fprintf fmt "[%a] %a %a --> %a %s" (pp_list "," pp_pdecl) pdecl pp_name
        cst
        (pp_list " " pp_prepattern)
        prepatterns pp_preterm prete name
  | None ->
      fprintf fmt "[%a] %a %a --> %a %s" (pp_list "," pp_pdecl) pdecl pp_ident
        id
        (pp_list " " pp_prepattern)
        prepatterns pp_preterm prete name
