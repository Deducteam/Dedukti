...
let mk_command lc =
  ...
  | OneStep te       ->
     ( match Env.one te with
       ...
       | Err e        -> Errors.fail_env_error e )
  | NSteps (n,te)   ->
     ( match Env.nsteps n te with
       | OK l  -> ignore (List.iteri (fun i t -> Format.printf "%i : %a\n" i print_term_nl t) l)
       | Err e -> Errors.fail_env_error e )
  | Conv (te1,te2)  ->
...
let print_command out c =
  ...
  | NSteps (n,te)    -> Format.fprintf out "#NSTEPS [%i]@ %a." n print_term te
  ...
  | InferSnf te      -> Format.fprintf out "#INFERSNF@ %a."   print_term te
  | Gdt (m0,v)       ->
      begin match m0 with
