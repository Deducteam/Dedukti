...
let rec subst_pt_rec k u = function
  ...
  | Meta n as t         -> t
  | Term (App args)     -> mk_partial_app (List.map (fun a -> subst_pt_rec k u (mk_partial a)) args)
  | Term (Lam (x,a,b))  -> mk_partial_lam x (subst_pt_rec k u (mk_partial a)) (subst_pt_rec (k+1) u (mk_partial b))
  | Term (Pi  (x,a,b))  -> mk_partial_pi  x (subst_pt_rec k u (mk_partial a)) (subst_pt_rec (k+1) u (mk_partial b)) 
