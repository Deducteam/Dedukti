...
let rec psubst_opt (args:LetCtx.t) (k:int) (t:term) : term =
  let nargs = LetCtx.len args in
  match t with
    | Type _ | Kind | Const _ | Meta _  -> t
    | DB (_,x,n) when (n >= (k+nargs))  -> mk_DB dloc x (n-nargs)
    | DB (_,_,n) when (n < k)           -> t
    | DB (_,_,n) (* (k<=n<(k+nargs)) *) ->
        begin match LetCtx.nth args (n-k) with
        | None -> t
        | Some (t',args') -> shift k (psubst_opt args' 0 t')
        end
    | Lam (_,x,a,b)                     ->
