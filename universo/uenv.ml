...
let _declare (l: loc) (id: ident) st ty : unit =
  try
    ...
    | Kind | Type _ ->
        if check_bump id then () else Signature.add_declaration !sg l id st ty
    | s -> raise (TypingError (SortExpected (ty, [], s)))
  with Signature.SignatureError SymbolNotFound _ ->
    Format.eprintf "dep:%a@." pp_ident id
...
let _define_op (l: loc) (id: ident) (te: term) (ty_opt: typ option) : unit =
  try
    ...
    | _ ->
        if check_bump id then ()
        else Signature.add_declaration !sg l id Signature.Static ty
  with Signature.SignatureError SymbolNotFound _ ->
    Format.eprintf "dep:%a@." pp_ident id
