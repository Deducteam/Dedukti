    ...
    match err with
      ...
      | ConvertibilityError (te,ctx,exp,inf) ->
          ...
          let inf = if !errors_in_snf then Env.unsafe_snf inf else inf in
            fail (get_loc te)
              ...
              Pp.pp_term te pp_context2 ctx Pp.pp_term exp Pp.pp_term inf
      | VariableNotFound (lc,x,n,ctx) ->
          fail lc "The variable '%a' was not found in context:\n"
            Pp.pp_term (mk_DB lc x n) Pp.pp_typed_context ctx
      | SortExpected (te,ctx,inf) ->
          let inf = if !errors_in_snf then Env.unsafe_snf inf else inf in
            fail (Term.get_loc te)
              "Error while typing '%a'%a.\nExpected: a sort.\nInferred: %a."
              Pp.pp_term te pp_context2 ctx Pp.pp_term inf
      ...
      | FreeVariableDependsOnBoundVariable (l,x,n,ctx,ty) ->
        fail l "Error while typing '%a[%i]'%a.\n\
                ...
                Infered type:%a." Pp.pp_ident x n pp_context2 ctx Pp.pp_term ty
  ...
  let open Dtree in
    match err with
      ...
      | NotEnoughArguments (lc,id,n,nb_args,exp_nb_args) ->
          fail lc "The variable '%a' is aPp.pplied to %i argument(s) (expected: at least %i)."
            Pp.pp_ident id nb_args exp_nb_args
      | HeadSymbolMismatch (lc,hd1,hd2) ->
          fail lc "Unexpected head symbol '%a' \ (expected '%a')."
            Pp.pp_ident hd1 Pp.pp_ident hd2
      | ArityMismatch (lc,id) ->
          fail lc
  ...
  let open Signature in
    match err with
      | FailToCompileModule (lc,md) ->
          fail lc "Fail to compile dependency '%a'." Pp.pp_ident md
      ...
      | SymbolNotFound (lc,md,id) ->
          fail lc "Cannot find symbol '%a.%a'." Pp.pp_ident md Pp.pp_ident id
      | AlreadyDefinedSymbol (lc,id) ->
          fail lc "Already declared symbol '%a'." Pp.pp_ident id
...
let fail_env_error = function
  ...
  | Env.KindLevelDefinition (lc,id) ->
    fail lc "Cannot add a rewrite rule for '%a' since it is a kind." Pp.pp_ident id
