...
let rec string_of_pattern s = function
...
and string_of_pattern_wp s = function
  | Pattern (_,_,args) as p when (Array.length args != 0) -> "(" ^ string_of_pattern s p ^ ")"
  |  p -> string_of_pattern s p 

let string_of_rule r = 
  "["^string_of_int r.nb ^"] "
  ^ string_of_pattern r.sub (Pattern (!Global.name,r.id,r.args)) 
  ^ " --> " ^ string_of_term r.ri
