...
let rec pp_ppattern out = function
  ...
  | PPattern (_,md,id,lst)      ->
      fprintf out "@[<hov2>%a@ %a@]" pp_pconst (md,id) (pp_list " " pp_ppattern) lst
...
let rec pp_term out = function
  ...
  | Meta (_,n)              -> pp_print_string out "_"
  | Var  (_,v) when !Global.debug_level > 0 -> Var.pp out v
  | Var  (_,v)                              -> pp_ident out (Var.ident v)
  | Const (_,m,v)         -> pp_const out (m,v)
  | App (f,a,[])        ->
      fprintf out "%a@ %a" pp_term_wp f pp_term_wp a
  | App (f,a,args)        ->
      fprintf out "%a@ %a@ %a"
        pp_term_wp f pp_term_wp a (pp_list " " pp_term_wp) args
  | Lam (_,v,a,f)         ->
      fprintf out "@[%a:@,@[%a@]@ => @[<hov1>%a@]@]" Var.pp v pp_term_wp a pp_term f
  | Pi  (_,None,a,b)      -> fprintf out "@[%a ->@ @[<hov1>%a@]@]" pp_term_wp a pp_term b
  | Pi  (_,Some v,a,b)    ->
      fprintf out "@[%a:@,@[%a@]@ -> @[<hov1>%a@]@]" Var.pp v pp_term_wp a pp_term b

and pp_term_wp out = function
  | Kind | Type _ | Var _ | Const _ as t  -> pp_term out t
  | t                                     -> fprintf out "@[<hv1>(%a)@]" pp_term t
...
let pp_subst ~sep out sigma =
  pp_list sep
    (fun out (v,t) -> fprintf out "@[<v>%a:@ @[%a@]@]" Var.pp v pp_term t)
...
let pp_context out ctx =
  pp_list "." (
    fun out (x,ty) ->
      ...
      else fprintf out "@[<v>%a:@ %a@]@," pp_ident x pp_term ty )
    out (IdentMap.bindings ctx.const2ty);
  pp_print_char out '\n';
  pp_subst ~sep:"." out ctx.var2ty
