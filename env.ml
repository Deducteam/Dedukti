  ...
  try
    ...
    let ver:string = Marshal.from_channel chan in
      if String.compare ver Global.version = 0 then
        ...
        Global.fail lc "Fail to open module '%a' (%a.dko has\
                                  been generated by another version of Dedukti). "
          pp_ident m pp_ident m
  ...
  ( if !Global.export then
      begin
        ...
        let env = H.find envs !Global.name in
          Marshal.to_channel out Global.version [Marshal.Closures] ;
