 ...
 *)
type matrix = {col_depth: int array; first: rule_infos; others: rule_infos list}
...
let filter_on_lambda c r =
  match (r.pats).(c) with LLambda _ | LJoker | LVar _ -> true | _ -> false
...
type case = CConst of int * name | CDB of int * int | CLam
...
type dtree =
  ...
  | Test of
      Rule.rule_name * matching_problem * constr list * term * dtree option

let rec pp_dtree t fmt dtree =
  let tab = String.make (t * 4) ' ' in
  match dtree with
  | Test (name, mp, [], te, None) ->
      fprintf fmt "{%a} (%a) %a" pp_rule_name name pp_matching_problem mp
        pp_term te
  | Test (name, mp, [], te, def) ->
      fprintf fmt "\n%s{%a} if true then (%a) %a\n%selse (%a) %a" tab
        pp_rule_name name pp_matching_problem mp pp_term te tab
        pp_matching_problem mp
        (pp_def (t + 1))
        def
  | Test (name, mp, lst, te, def) ->
      let aux out = function
        ...
        | Bracket (i, j) ->
            fprintf out "{%a} %a =b %a" pp_rule_name name pp_term
              (mk_DB dloc dmark i) pp_term j
      in
      fprintf fmt "\n%sif %a then (%a) %a\n%selse (%a) %a" tab
        (pp_list " and " aux) lst pp_matching_problem mp pp_term te tab
        pp_matching_problem mp
        (pp_def (t + 1))
        def
  | Switch (i, cases, def) ->
      let pp_case out = function
        | CConst (_, cst), g ->
            fprintf out "\n%sif $%i=%a then %a" tab i pp_name cst
              (pp_dtree (t + 1))
              g
        | CLam, g ->
            fprintf out "\n%sif $%i=Lambda then %a" tab i (pp_dtree (t + 1)) g
        | CDB (_, n), g ->
            fprintf out "\n%sif $%i=DB[%i] then %a" tab i n
              (pp_dtree (t + 1))
              g
      in
      fprintf fmt "%a\n%sdefault: %a" (pp_list "" pp_case) cases tab
        (pp_def (t + 1))
        def
...
let pp_rw fmt (cst, i, g) =
  fprintf fmt "GDT for '%a' with %i argument(s): %a" pp_name cst i pp_dtree g
  ...
  Array.iteri
    (fun i p ->
      ...
      | LVar (_, n, lst) ->
          ...
          arr1.(n - k) <- pos ;
          if lst = [] then arr2.(n - k) <- (pos, LList.nil)
          else (
            mp := true ;
...
let of_rules (rs: rule_infos list) : (int * dtree, dtree_error) error =
  try
    let mx = mk_matrix rs in
    OK (Array.length mx.first.pats, to_dtree mx)
