...
let print_db_or_underscore out (x, n) =
  if is_dummy_ident x then Format.fprintf out "_" else print_ident out x
...
let fresh_name names base =
  if List.mem base names then (
    ...
    while List.mem (name !i) names do incr i done ;
    name !i )
  else base
...
let rec subst map = function
  ...
  | App (f, a, args) ->
      mk_App (subst map f) (subst map a) (List.map (subst map) args)
  | Lam (l, x, None, f) ->
      let x' = fresh_name map x in
      mk_Lam l x' None (subst (x' :: map) f)
  | Lam (l, x, Some a, f) ->
      let x' = fresh_name map x in
      mk_Lam l x' (Some (subst map a)) (subst (x' :: map) f)
  | Pi (l, x, a, b) ->
      let x' = if is_dummy_ident x then x else fresh_name map x in
...
let rec print_term out = function
  ...
  | App (f, a, args) ->
      Format.fprintf out "@[<hov2>%a@]"
        (print_list " " print_term_wp)
        (f :: a :: args)
  | Lam (_, x, None, f) ->
      Format.fprintf out "@[%a =>@ @[%a@]@]" print_ident x print_term f
  | Lam (_, x, Some a, f) ->
      Format.fprintf out "@[%a:@,%a =>@ @[%a@]@]" print_ident x print_term_wp a
        print_term f
...
let rec print_pattern out = function
  ...
  | Var (_, id, i, lst) ->
      Format.fprintf out "%a %a" print_db_or_underscore (id, i)
  ...
  | Pattern (_, cst, pats) ->
      Format.fprintf out "%a %a" print_const cst
        (print_list " " print_pattern_wp)
        pats
  | Lambda (_, x, p) ->
      Format.fprintf out "@[%a => %a@]" print_ident x print_pattern p
...
let print_untyped_rule fmt (rule: untyped_rule) =
  ...
  Format.fprintf fmt
    ...
    (List.filter (fun (_, id) -> is_regular_ident id) rule.ctx)
...
let print_typed_rule out (rule: typed_rule) =
  ...
  Format.fprintf out
    ...
    (print_list ", " print_decl)
    rule.ctx print_pattern rule.pat print_term rule.rhs
...
let print_red_cfg fmt strat =
  ...
  | {strategy= Reduction.Hnf; nb_steps= Some i} ->
      Format.fprintf fmt "[HNF,%i]" i
  | {strategy= Reduction.Whnf; nb_steps= None} -> Format.fprintf fmt "[WHNF]"
  | {strategy= Reduction.Whnf; nb_steps= Some i} ->
      Format.fprintf fmt "[WHNF,%i]" i
...
let print_entry fmt e =
  ...
  | Def (_, id, opaque, ty, te) -> (
      ...
      match ty with
      | None ->
          fprintf fmt "@[<hv2>%s %a@ :=@ %a.@]@.@." key print_ident id
            print_term te
      | Some ty ->
          fprintf fmt "@[<hv2>%s %a :@ %a@ :=@ %a.@]@.@." key print_ident id
            print_term ty print_term te )
  | Rules rs ->
