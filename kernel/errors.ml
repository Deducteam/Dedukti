...
let fail_typing_error err =
  ...
  | CannotSolveConstraints (r,cstr) ->
    fail (get_loc_pat r.pat)
      ...
      Pp.print_untyped_rule r (pp_list "\n" (fun out (_,t1,t2) -> fprintf out "%a ~~ %a" Pp.print_term t1 Pp.print_term t2)) cstr
  ...
  | FreeVariableDependsOnBoundVariable (l,x,n,ctx,ty) ->
    fail l "Error while typing '%a[%i]'%a.\n\
            ...
            Infered type:%a." Pp.print_ident x n pp_typed_context ctx Pp.print_term ty
