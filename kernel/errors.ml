  ...
  let open Typing in
    match err with
      ...
      | FreeVariableDependsOnBoundVariable (l,x,n,ctx,ty) ->
        fail l "Error while typing '%s[%i]'%a.\n\
                The type is not allowed to refer to bound variables.\n\
                Infered type:%a." x n pp_typed_context ctx pp_term ty
    ...
    match err with
      | HeadSymbolMismatch (lc,cst1,cst2) ->
          fail lc "Unexpected head symbol '%a' \ (expected '%a')."
            Name.pp_ident cst1 Name.pp_ident cst2
      ...
      | ArityInnerMismatch (lc,rid, id) ->
          fail lc
            "The definable symbol '%s' inside the rewrite rules for \ '%a' should have the same arity when they are on the same column."
             id Name.pp_ident rid
...
let fail_rule_error err =
  ...
  | DistinctBoundVariablesExpected (lc,x) ->
    fail lc "The pattern of the rule is not a Miller pattern. The variable '%s' should be applied to distinct variables." x
  | UnboundVariable (lc,x,pat) ->
    fail lc "The variables '%s' does not appear in the pattern '%a'."
      x pp_pattern pat
  | AVariableIsNotAPattern (lc,id) ->
    fail lc "A variable is not a valid pattern."
  | NotEnoughArguments (lc,var,n,nb_args,exp_nb_args) ->
    fail lc "The variable '%s' is applied to %i argument(s) (expected: at least %i)."
     var nb_args exp_nb_args
  | NonLinearRule r ->
  ...
  let open Signature in
    match err with
      | FailToCompileModule (lc,md) ->
          fail lc "Fail to compile dependency '%a'." Name.pp_mident md
      ...
      | SymbolNotFound (lc,cst) ->
          fail lc "Cannot find symbol '%a'." Name.pp_ident cst
      | AlreadyDefinedSymbol (lc,id) ->
          fail lc "Already declared symbol '%a'." Name.pp_ident id
...
Add the keyword 'def' to its declaration to make the symbol '%a' definable."
            Name.pp_ident id Name.pp_ident id
      ...
      | ConfluenceErrorImport (lc,md,cerr) ->
        fail lc "Confluence checking failed when importing the module '%a'.\n%a"
          Name.pp_mident md pp_cerr cerr
...
let fail_env_error = function
  ...
  | Env.KindLevelDefinition (lc,id) ->
    fail lc "Cannot add a rewrite rule for '%a' since it is a kind." Name.pp_ident id
