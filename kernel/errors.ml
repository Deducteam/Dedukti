...
let snf sg t =
  if !errors_in_snf then Reduction.reduction snf_config sg t else t
...
let fail_typing_error sg err =
  ...
  | ConvertibilityError (te, ctx, exp, inf) ->
      fail (get_loc te)
        ...
        pp_typed_context ctx pp_term (snf sg exp) pp_term (snf sg inf)
  ...
  | SortExpected (te, ctx, inf) ->
      fail (Term.get_loc te)
        "Error while typing '%a'%a.\nExpected: a sort.\nInferred: %a." pp_term
        te pp_typed_context ctx pp_term (snf sg inf)
...
let fail_env_error sg = function
  | Env.EnvErrorSignature e -> fail_signature_error e
  | Env.EnvErrorType e -> fail_typing_error sg e
