...
let snf t =
  if !errors_in_snf then Env.unsafe_reduction ~red:snf_config t else t
...
let fail_typing_error err =
  ...
  | ConvertibilityError (te, ctx, exp, inf) ->
      fail (get_loc te)
        "Error while typing '%a'%a.\nExpected: %a\nInferred: %a." pp_term te
        pp_typed_context ctx pp_term (snf exp) pp_term (snf inf)
  | VariableNotFound (lc, x, n, ctx) ->
      fail lc "The variable '%a' was not found in context:\n" pp_term
        (mk_DB lc x n) pp_typed_context ctx
  | SortExpected (te, ctx, inf) ->
      fail (Term.get_loc te)
        "Error while typing '%a'%a.\nExpected: a sort.\nInferred: %a." pp_term
        te pp_typed_context ctx pp_term (snf inf)
  ...
  | InexpectedKind (te, ctx) ->
      fail (get_loc te)
        "Error while typing '%a'%a.\n\
         ...
         Inferred: Kind." pp_term te pp_typed_context ctx
  | DomainFreeLambda lc ->
      fail lc "Cannot infer the type of domain-free lambda."
  | CannotInferTypeOfPattern (p, ctx) ->
      fail (get_loc_pat p)
        "Error while typing '%a'%a.\nThe type could not be infered." pp_pattern
        p pp_typed_context ctx
  | CannotSolveConstraints (r, cstr) ->
      fail (get_loc_pat r.pat)
        "Error while typing the rewrite rule\n\
         ...
         %a" pp_untyped_rule r
        (pp_list "\n" (fun out (_, t1, t2) ->
             fprintf out "%a ~~ %a" pp_term t1 pp_term t2 ))
        cstr
  | BracketError1 (te, ctx) ->
      fail (get_loc te)
        "Error while typing the term { %a }%a.\n\
         Brackets can only contain variables occuring on their left and \
         cannot contain bound variables." pp_term te pp_typed_context ctx
  | BracketError2 (te, ctx, ty) ->
      fail (get_loc te)
        "Error while typing the term { %a }%a.\n\
         The type of brackets can only contain variables occuringon their \
         left and cannot contains bound variables." pp_term te pp_typed_context
        ctx
  ...
  | Convertible (l, t1, t2) ->
      fail l "Assertion error. Given terms are convertible: '%a' and '%a'"
        pp_term t1 pp_term t2
  | Inhabit (l, t1, t2) ->
      fail l "Assertion error. '%a' is of type '%a'" pp_term t1 pp_term t2
...
let fail_rule_error err =
  ...
  | BoundVariableExpected pat ->
      fail (get_loc_pat pat)
        "The pattern of the rule is not a Miller pattern. The pattern '%a' is \
         not a bound variable." pp_pattern pat
  | VariableBoundOutsideTheGuard te ->
      fail (get_loc te)
        "The term '%a' contains a variable bound outside the brackets." pp_term
        te
  | DistinctBoundVariablesExpected (lc, x) ->
      fail lc
        "The pattern of the rule is not a Miller pattern. The variable '%a' \
         should be applied to distinct variables." pp_ident x
  ...
  | NonLinearRule r ->
      fail (Rule.get_loc_pat r.pat)
        "Non left-linear rewrite rule:\n\
         %a.\n\
         Maybe you forgot to pass the -nl option." pp_untyped_rule r
  | NonLinearNonEqArguments (lc, arg) ->
      fail lc
        "For each occurence of the free variable %a, the symbol should be \
         applied to the same number of arguments" pp_ident arg


let pp_cerr out err =
  let open Confluence in
  match err with
  | NotConfluent cmd -> fprintf out "Checker's answer: NO.\nCommand: %s" cmd
  | MaybeConfluent cmd ->
      fprintf out "Checker's answer: MAYBE.\nCommand: %s" cmd
  | CCFailure cmd -> fprintf out "Checker's answer: ERROR.\nCommand: %s" cmd
...
let fail_signature_error err =
  ...
  | UnmarshalBadVersionNumber (lc, md) ->
      fail lc
        "Fail to open module '%s' (file generated by a different version?)." md
  | UnmarshalSysError (lc, md, msg) ->
      fail lc "Fail to open module '%s' (%s)." md msg
  | UnmarshalUnknown (lc, md) -> fail lc "Fail to open module '%s'." md
  | SymbolNotFound (lc, cst) -> fail lc "Cannot find symbol '%a'." pp_name cst
  ...
  | CannotAddRewriteRules (lc, id) ->
      fail lc
        "Cannot add rewrite rules for the static symbol '%a'.\n\
         Add the keyword 'def' to its declaration to make the symbol '%a' \
         definable." pp_ident id pp_ident id
...
let fail_env_error = function
  ...
  | Env.KindLevelDefinition (lc, id) ->
      fail lc "Cannot add a rewrite rule for '%a' since it is a kind." pp_ident
        id
