...
let pp_state ?(if_ctx= true) ?(if_stack= true) fmt {ctx; term; stack} =
  if if_ctx then fprintf fmt "{ctx=[%a];@." pp_env ctx
  else fprintf fmt "{ctx=[...](%i);@." (LList.len ctx) ;
  fprintf fmt "term=%a;@." pp_term term ;
  if if_stack then fprintf fmt "stack=%a}@." pp_stack stack
...
let rec safe_find m v = function
  | [] -> None
  | (_, m', v', tr) :: tl ->
      if ident_eq v v' && ident_eq m m' then Some tr else safe_find m v tl
...
type convertibility_test = Signature.t -> bool -> (term * term) list -> bool

let solve (sg: Signature.t) (reduce: rw_strategy) (depth: int)
    (pbs: int LList.t) (te: term) : term =
  try Matching.solve depth pbs te with Matching.NotUnifiable ->
    Matching.solve depth pbs (reduce sg te)


let unshift (sg: Signature.t) (reduce: rw_strategy) (q: int) (te: term) =
  try Subst.unshift q te with Subst.UnshiftExn ->
...
let get_context_syn (sg: Signature.t) (forcing: rw_strategy) (stack: stack)
  ...
  try
    Some
      (LList.map
         (fun p ->
           if p.depth = 0 then lazy (term_of_state (List.nth stack p.position))
           else
             Lazy.from_val
               (unshift sg forcing p.depth
                  (term_of_state (List.nth stack p.position))) )
         ord)
...
let get_context_mp (sg: Signature.t) (forcing: rw_strategy) (stack: stack)
  ...
  let aux ((pos, dbs): abstract_problem) : term Lazy.t =
    let res =
      solve sg forcing pos.depth dbs
        (term_of_state (List.nth stack pos.position))
    in
    Lazy.from_val (Subst.unshift pos.depth res)
  in
  try Some (LList.map aux pb_lst) with
  | Matching.NotUnifiable -> None
  | Subst.UnshiftExn -> assert false


let rec test (sg: Signature.t) (convertible: convertibility_test) (ctx: env) r
    (constrs: constr list) : bool =
  let ism =
    match r with
    | Gamma (_, n) -> if string_of_mident (md n) = "cic" then true else false
    | _ -> false
  in
  match constrs with
  | [] -> true
  | (Linearity (i, j)) :: tl ->
      ...
      let is_conv =
        convertible sg ism
          [ ( term_of_state {ctx; term= t1; stack= []}
            , term_of_state {ctx; term= t2; stack= []} ) ]
      in
      if is_conv then test sg convertible ctx r tl else false
  | (Bracket (i, t2)) :: tl ->
      let t1 = mk_DB dloc dmark i in
      let is_conv =
        convertible sg ism
          [ ( term_of_state {ctx; term= t1; stack= []}
            , term_of_state {ctx; term= t2; stack= []} ) ]
      in
      if is_conv then test sg convertible ctx r tl
      else
...
let rec find_case (st: state) (cases: (case * dtree) list)
  ...
  | {term= Const (_, n); stack}, (CConst (nargs, n'), tr) :: tl ->
      ...
      * of the expected size. *)
      if name_eq n n' && List.length stack == nargs then Some (tr, stack)
...
let gamma_rw (sg: Signature.t) (convertible: convertibility_test)
  ...
  let rec rw stack = function
    | Switch (i, cases, def) -> (
        ...
        | None ->
            None )
    | Test (r, Syntactic ord, eqs, right, def) -> (
      match get_context_syn sg forcing stack ord with
      | None -> bind_opt (rw stack) def
      | Some ctx ->
          if test sg convertible ctx r eqs then Some (ctx, right)
          else bind_opt (rw stack) def )
    | Test (r, MillerPattern lst, eqs, right, def) ->
...
let rec state_whnf (sg: Signature.t) (st: state) : state =
  ...
  | {ctx; term= Const (l, n); stack} ->
      ...
      | Some (i, g) ->
        ...
        | None -> st
        | Some (s1, s2) ->
          match gamma_rw sg are_convertible_lst snf state_whnf s1 g with
          | None -> st
...
and are_convertible_lst sg ism (lst: (term * term) list) : bool =
  match lst with
  | [] -> true
  | (t1, t2) :: lst ->
    match
...
let rec hnf sg t =
  match whnf sg t with
  | (Kind | Const _ | DB _ | Type _ | Pi (_, _, _, _) | Lam (_, _, _, _)) as t' ->
      t'
  | App (f, a, lst) -> mk_App (hnf sg f) (hnf sg a) (List.map (hnf sg) lst)


let reduction = function Hnf -> hnf | Snf -> snf | Whnf -> whnf

(* n-steps reduction on state *)
let state_nsteps (sg: Signature.t) (strat: red_strategy) (steps: int)
    (state: state) =
  let rec aux ((red, st): int * state) : int * state =
    if red <= 0 then (0, st)
      ...
      | {ctx; term= Pi (l, x, a, b)} ->
          ...
          let state_b =
            {ctx= LList.cons (lazy snf_a) ctx; term= b; stack= []}
          in
      ...
      | {ctx; term= App (f, a, lst); stack= s}
        when strat <> Snf ->
          let tl' =
            List.rev_map (fun t -> {ctx; term= t; stack= []}) (a :: lst)
          in
