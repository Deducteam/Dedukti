...
let rec shift_rec (r: int) (k: int) : term -> term = function
  ...
  | App (f, a, args) ->
      mk_App (shift_rec r k f) (shift_rec r k a)
        (List.map (shift_rec r k) args)
  | Lam (_, x, a, f) ->
      mk_Lam dloc x (map_opt (shift_rec r k) a) (shift_rec r (k + 1) f)
...
let unshift q te =
  let rec aux k = function
    ...
    | DB (l, x, n) -> if n - k < q then raise UnshiftExn else mk_DB l x (n - q)
...
module Subst = struct
  ...
  let apply (sigma: t) (te: term) (q: int) : term =
    let rec aux q = function
      ...
      | DB (_, _, k) as t (*when k>=q*) -> (
        try shift q (snd (IntMap.find (k - q) sigma)) with Not_found -> t )
      | App (f, a, args) -> mk_App (aux q f) (aux q a) (List.map (aux q) args)
      | Lam (l, x, Some ty, te) ->
          mk_Lam l x (Some (aux q ty)) (aux (q + 1) te)
  ...
  let pp (fmt: formatter) (sigma: t) : unit =
    IntMap.iter
      (fun i (x, t) -> fprintf fmt "( %a[%i] = %a )" pp_ident x i pp_term t)
      sigma


  let add (sigma: t) (x: ident) (n: int) (t: term) : t option =
    assert (not (IntMap.mem n sigma)) ;
    if occurs n t then None else Some (IntMap.add n (x, t) sigma)


  let rec mk_idempotent (sigma: t) : t =
    let sigma2 : t = IntMap.map (fun (x, te) -> (x, apply sigma te 0)) sigma in
    if IntMap.equal (fun a b -> term_eq (snd a) (snd b)) sigma sigma2 then
      sigma
