...
type arg_pos =
  { position: int
        (** position of the argument from left to right of the pattern *)
...
type dtree =
  | Switch of int * (case * dtree) list * dtree option
      (** Switch [i] [(case_0,tree_0) ; ... ; (case_n, tree_n)] [tree_opt] test if the [i] arg of a pattern can be match with one of the case of the list. if it does then look at the corresponding tree, otherwise, look at the default tree *)
  | Test of
      rule_name * matching_problem * constr list * Term.term * dtree option
