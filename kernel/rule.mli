...
type constr =
  | Linearity of int * int
      (** DB indices [i] and [j] of the pattern should be convertible *)
  | Bracket of int * term
      (** DB indices [i] should be convertible to the term [te] *)

(** {2 Rewrite Rules} *)

type rule_name =
  | Delta of name  (** Rules associated to the definition of a constant *)
  | Gamma of bool * name
