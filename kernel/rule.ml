...
type 'a rule =
  {name: rule_name; ctx: 'a; pat: pattern; cond: condition option; rhs: term}
...
let pp_typed_context fmt ctx =
  pp_idents fmt (List.map (fun (_, a, _) -> a) ctx)
...
let pp_untyped_rule fmt (rule: untyped_rule) =
  fprintf fmt " {%a} [%a] %a --> %a" pp_rule_name rule.name pp_untyped_context
    rule.ctx pp_pattern rule.pat pp_term rule.rhs


let pp_typed_rule fmt (rule: typed_rule) =
  fprintf fmt " {%a} [%a] %a --> %a" pp_rule_name rule.name pp_typed_context
    rule.ctx pp_pattern rule.pat pp_term rule.rhs
...
type pattern_info =
  {constraints: constr list; context_size: int; arity: int array}
...
let bracket_ident = mk_ident "{_}"

(* FIXME: can this be replaced by dmark? *)
...
let check_patterns (esize: int) (pats: pattern list)
  ...
  let fresh_var ar =
    (* DB indice for a fresh context variable with given arity *)
  ...
  let rec aux (k: int) (pat: pattern) : wf_pattern =
    ...
    | Var (l, x, n, args) (* Context variable (n>=k)  *) ->
        ...
        if IntHashtbl.mem arity (n - k) then (
          if nb_args' <> IntHashtbl.find arity (n - k) then
            raise (RuleExn (NonLinearNonEqArguments (l, x)))
          else
            let nvar = fresh_var nb_args' in
            constraints
            := Condition
                 (mk_DB dloc dmark nvar, mk_DB dloc dmark (n - k), false)
               :: !constraints ;
            LVar (x, nvar + k, args') )
        else
    ...
    | Brackets t ->
        let unshifted =
          try Subst.unshift k t with Subst.UnshiftExn ->
            raise (RuleExn (VariableBoundOutsideTheGuard t))
        in
        let nvar = fresh_var 0 in
        constraints
        := Condition (mk_DB dloc dmark nvar, unshifted, true) :: !constraints ;
        LVar (bracket_ident, nvar + k, [])
    | Pattern (_, n, args) ->
        LPattern (n, Array.of_list (List.map (aux k) args))
  in
...
let check_nb_args (arity: int array) (rhs: term) : unit =
  let check l id n k nargs =
    ...
    if nargs < expected_args then
      raise (RuleExn (NotEnoughArguments (l, id, n, nargs, expected_args)))
  in
  let rec aux k = function
    | Kind | Type _ | Const _ -> ()
...
let to_rule_infos r =
  ...
  if List.length infos.constraints > 0 then
    if !allow_conditional then debug 1 "Conditional rewrite rule detected"
