...
type 'a rule = {name: rule_name; ctx: 'a; pat: pattern; rhs: term}
...
type constr = Linearity of int * int | Bracket of int * term
...
let to_rule_infos (r: untyped_rule) : (rule_infos, rule_error) error =
  ...
  try
    ...
    if not (is_linear infos.constraints) then
      if !allow_non_linear then debug 1 "Non-linear Rewrite Rule detected"
