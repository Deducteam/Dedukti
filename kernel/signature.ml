...
let marshal (file: string) (deps: string list) (env: rw_infos HId.t)
    (ext: rule_infos list list) : bool =
  try
...
let rec find_dko_in_path name = function
  ...
  | dir :: path ->
      ...
      if file_exists filename then open_in filename
...
let unmarshal (lc: loc) (m: string)
    : string list * rw_infos HId.t * rule_infos list list =
  try
...
and add_rule_infos sg (lst: rule_infos list) : unit =
  ...
  | r :: _ as rs ->
      let env =
        try HMd.find sg.tables (md r.cst) with Not_found ->
          import sg r.l (md r.cst) ;
          HMd.find sg.tables (md r.cst)
      in
      let infos =
        try HId.find env (id r.cst) with Not_found ->
          raise (SignatureError (SymbolNotFound (r.l, r.cst)))
      in
      let ty = infos.ty in
      ...
      | OK (n, tree) ->
          HId.add env (id r.cst)
            {stat= infos.stat; ty; rule_opt_info= Some (rules, n, tree)}
      | Err e -> raise (SignatureError (CannotBuildDtree e))


(******************************************************************************)

let get_deps sg : string list =
  (*only direct dependencies*)
  HMd.fold
    (fun md _ lst ->
      if mident_eq md sg.name then lst else string_of_mident md :: lst )
    sg.tables []
...
let get_infos sg lc cst =
  let md = md cst in
  let env =
    try HMd.find sg.tables md with Not_found ->
      import sg lc md ; HMd.find sg.tables md
  in
  try HId.find env (id cst) with Not_found ->
    raise (SignatureError (SymbolNotFound (lc, cst)))


let is_injective sg lc cst =
  match (get_infos sg lc cst).stat with Static -> true | Definable -> false
...
let get_dtree sg rule_filter l cst =
  ...
  | Some (rules, i, tr) ->
    ...
    | Some f ->
        let rules' =
          List.filter (fun (r: Rule.rule_infos) -> f r.name) rules
        in
        if List.length rules' == List.length rules then Some (i, tr)
        else
...
let add_declaration sg lc v st ty =
  ...
  if HId.mem env v then raise (SignatureError (AlreadyDefinedSymbol (lc, v)))
  else HId.add env v {stat= st; ty; rule_opt_info= None}
