...
and check sg (ctx: typed_context) (te: term) (ty_exp: typ) : unit =
  ...
  | Lam (l, x, Some a, b) -> (
    ...
    | Pi (_, _, a', ty_b) ->
        ...
        if not (Reduction.are_convertible sg a a') then
          raise (TypingError (ConvertibilityError (mk_DB l x 0, ctx, a', a)))
...
and check_app sg (ctx: typed_context) ((f, ty_f): term * typ) (arg: term)
    : term * typ =
  match whnf sg ty_f with
  | Pi (_, _, a, b) ->
      let _ = check sg ctx arg a in
...
let safe_add_to_list q lst args1 args2 =
  try Some (add_to_list q lst args1 args2) with Invalid_argument _ -> None


module SS = Subst.Subst

let unshift_reduce sg q t =
  try Some (Subst.unshift q t) with Subst.UnshiftExn ->
    try Some (Subst.unshift q (snf sg t)) with Subst.UnshiftExn -> None


let rec pseudo_u sg (sigma: SS.t) : (int * term * term) list -> SS.t option =
  function
  | [] -> Some sigma
  | (q, t1, t2) :: lst ->
      ...
      if term_eq t1' t2' then pseudo_u sg sigma lst
        ...
        | DB (l1, x1, n1), DB (l2, x2, n2) when n1 >= q && n2 >= q -> (
            let x, n, t =
              if n1 < n2 then (x1, n1, mk_DB l2 x2 (n2 - q))
              else (x2, n2, mk_DB l1 x1 (n1 - q))
            in
        ...
        | App (Const (l, cst), _, _), _
          when not (Signature.is_injective sg l cst) ->
            debug 2 "Ignoring non injective constraint: %a ~ %a" pp_term t1'
              pp_term t2' ;
            pseudo_u sg sigma lst
        | _, App (Const (l, cst), _, _)
          when not (Signature.is_injective sg l cst) ->
            debug 2 "Ignoring non injective constraint: %a ~ %a" pp_term t1'
              pp_term t2' ;
            pseudo_u sg sigma lst
        | App (f, a, args), App (f', a', args') -> (
           ...
           * | Const name when (is_injective name) *)
            safe_add_to_list q lst args args'
          with
          | None -> None
...
let pc_get (delta: partial_context) (n: int) : term option =
  if n < delta.padding then None
    ...
    let _, _, ty = List.nth (LList.lst delta.pctx) (n - delta.padding) in
    Some (Subst.shift (n + 1) ty)


let pc_add (delta: partial_context) (n: int) (l: loc) (id: ident) (ty0: typ)
    : partial_context =
  assert (n == delta.padding - 1 && n >= 0) ;
  let ty = Subst.unshift (n + 1) ty0 in
  {padding= delta.padding - 1; pctx= LList.cons (l, id, ty) delta.pctx}
...
let pc_to_context_wp (delta: partial_context) : typed_context =
  ...
  let rec aux lst n =
    if n <= 0 then lst else aux ((dloc, dmark, dummy) :: lst) (n - 1)
  in
...
let pp_pcontext fmt delta =
  ...
  List.iteri
    (fun i (_, x, ty) -> fprintf fmt "%a[%i]:%a\n" pp_ident x i pp_term ty)
    lst ;
...
let unshift_n sg n te =
  try Subst.unshift n te with Subst.UnshiftExn -> Subst.unshift n (snf sg te)
...
and check_pattern sg (delta: partial_context) (sigma: context2) (exp_ty: typ)
  ...
  | Lambda (l, x, p) -> (
    ...
    | exp_ty ->
        let ctx = LList.lst sigma @ pc_to_context_wp delta in
        raise
          (TypingError (ProductExpected (pattern_to_term pat, ctx, exp_ty))) )
  | Brackets te ->
      let te2 =
...
let rec pp_term_j k fmt = function
  ...
  | Lam (_, x, Some a, f) ->
      fprintf fmt "%a:%a => %a" pp_ident x (pp_term_wp_j (k + 1)) a pp_term f
  | Pi (_, x, a, b) ->
      fprintf fmt "%a:%a -> %a" pp_ident x (pp_term_wp_j (k + 1)) a pp_term b
...
let check_rule sg (rule: untyped_rule) : typed_rule =
  ...
  let sub =
    match pseudo_u sg SS.identity lst with
  ...
  let ri2, ty_le2, ctx2 =
    if SS.is_identity sub then (rule.rhs, ty_le, LList.lst delta.pctx)
      ...
      | None ->
          ...
          SS.iter
            (fun i (id, te) ->
              debug 2 "Try replacing '%a[%i]' by '%a'" pp_ident id i
                (pp_term_j 0) te )
            sub ;
          raise (TypingError (NotImplementedFeature (get_loc_pat rule.pat)))
  in
  check sg ctx2 ri2 ty_le2 ;
  debug 2 "[ %a ] %a --> %a" pp_context_inline ctx2 pp_pattern rule.pat pp_term
    ri2 ;
  {name= rule.name; ctx= ctx2; pat= rule.pat; rhs= rule.rhs}
