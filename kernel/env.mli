...
val get_dtree :
  loc -> name -> ((int * Dtree.dtree) option, signature_error) error
...
val declare :
  loc -> ident -> Signature.staticity -> term -> (unit, env_error) error
...
val add_rules :
  Rule.untyped_rule list -> (Rule.typed_rule list, env_error) error
...
val reduction :
  ?ctx:typed_context -> ?red:Reduction.red_cfg -> term
  -> (term, env_error) error

val are_convertible :
  ?ctx:typed_context -> term -> term -> (bool, env_error) error
