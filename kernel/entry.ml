...
let pp_entry fmt e =
  ...
  | Def (_, id, opaque, ty, te) -> (
      ...
      match ty with
      | None ->
          fprintf fmt "@[<hv2>%s %a@ :=@ %a.@]@.@." key pp_ident id pp_term te
      | Some ty ->
          fprintf fmt "@[<hv2>%s %a :@ %a@ :=@ %a.@]@.@." key pp_ident id
            pp_term ty pp_term te )
  | Rules rs ->
  ...
  | Name (_, _) -> ()
  | Require (_, m) -> fprintf fmt "#REQUIRE %a.@." pp_mident m
